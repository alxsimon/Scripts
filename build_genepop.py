...
parser = argparse.ArgumentParser(
    description="Transform csv file in nucleotide genotype format to a genepop format."
)
parser.add_argument(
    "infile",
...
outfile = args.outdir + args.infile[args.infile.rfind("/") :].replace(
    ".csv", "_genepop.txt"
)
...
def get_alleles(column_id, dataframe):
    ...
    alleles = unique(
        [item for sublist in [x.split("/") for x in genotypes] for item in sublist]
    )
