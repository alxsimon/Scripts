												 ...
												 pdf.height = 9.5, pdf.width = 10) 
{
	...
	if (!is.null(marker.order) & (length(marker.order) < length(loci.touse))){
		loci.touse <- 1:length(marker.order)
	}
	...
	if (is.null(marker.order)) {
		marker.order <- 1:(dim(count.matrix)[1])
	}
