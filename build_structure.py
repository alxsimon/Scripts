...
parser = argparse.ArgumentParser(
    description="Transform csv file in nucleotide genotype format to a structure input file."
)
parser.add_argument(
    "infile",
    help="CSV file containing individuals index in 1st column, population id in 2nd column and genotypes after. 1st row is marker ids.",
)
parser.add_argument("--outfile", help="Output directory")
parser.add_argument(
    "--recodepop", help="Recode populations or not (default 1)", type=int, default=1
)
parser.add_argument(
    "--locprior", help="Define the LOCPRIOR parameter in Structure", type=int, default=0
)
parser.add_argument(
    "--pfrompopflagonly",
    help="Define the PFROMPOPFLAGONLY parameter in Structure",
    type=int,
    default=0,
)
parser.add_argument(
    "--burnin", help="Define the BURNIN parameter in Structure", type=int, default=20000
)
parser.add_argument(
    "--numreps",
    help="Define the NUMREPS parameter in Structure",
    type=int,
    default=80000,
)
...
parser.add_argument(
    "--noadmix",
    help="use the NOADMIX model, does not work with Linkage model",
    default=0,
)
...
if args.recodepop == 1:
    with open(outpop, "w", newline="") as popFile:
        csvwriter = csv.writer(popFile, delimiter="\t", lineterminator="\n")
        for index, pop in enumerate(uniqPop):
...
if args.genmap is not None:
    extraparNames.extend(
        ["LOG10RSTART", "LOG10RMIN", "LOG10RMAX", "LOG10RPROPSD", "ADMBURNIN"]
    )
